...
public interface OrderService {
    ...
    // Driver accept the order, the order can only be accepted if it already started
    // and not in delivery
    int acceptOrder(String id, String driverId);

    // Driver finish the order, the order can only be finished if it is in delivery
    // and not finished
    ...
    // Customer check all active orders, no matter there is or isn't a driver and
    // the order isn't finished
